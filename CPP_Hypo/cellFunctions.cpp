...
extern uniform_int_distribution<int> distribution10k,distribution100,distribution10,distribution8,distribution3;
...
void move(int orient, int* x, int* ym, int (&cellGrid)[101][101]);

float cytokineProductionRule(int ruleRow, int cellIndex, float tnfr, float il1r, float multiplier, vector <EC>&ecArray);
float cytokineComboRule(int ruleRow, int cellIndex, float tnfr, float il1r, vector <EC>&ecArray);

void heal(int index, vector<EC>&ecArray);
void applyAntibiotics(float antibioticMultiplier, vector<EC>&ecArray);

// Function Definitions

// Functions that all the cells use
void adjustOrientation(int* orientation, int leftOrRight){
...
void getRuleMatrix(float* internalParam){
  int i,j,k=0;
...
float cytokineProductionRule(int ruleRow, int index, float tnfr, float il1r, float multiplier, vector<EC>& ecArray){
  int i;
  float tempSum=0;
  float currentCyto[16];
  currentCyto[0]=ecArray[index].PAF;
  currentCyto[1]=ecArray[index].IL8;
  currentCyto[2]=ecArray[index].IL1ra;
  currentCyto[3]=ecArray[index].TNF;
  currentCyto[4]=ecArray[index].IL1;
  currentCyto[5]=tnfr;
  currentCyto[6]=il1r;
  currentCyto[7]=ecArray[index].IL1ra;
  currentCyto[8]=ecArray[index].sTNFr;
  currentCyto[9]=ecArray[index].sIL1r;
  currentCyto[10]=ecArray[index].GCSF;
  currentCyto[11]=ecArray[index].IL12;
  currentCyto[12]=ecArray[index].IL10;
  currentCyto[13]=ecArray[index].IL4;
  currentCyto[14]=ecArray[index].IFNg;
  currentCyto[15]=ecArray[index].endotoxin;
...
//  cout<<"Const="<<RM[ruleRow][16]<<"\n";
  ...
  if(multiplier<=1){
    tempSum=tempSum*multiplier;
  }
  else{
    tempSum=tempSum+(multiplier-1);
  }
  return tempSum;
}

float cytokineComboRule(int ruleRow, int index, float tnfr, float il1r, vector<EC>&ecArray){
  int i;
  float tempSum=0;
  float currentCyto[16];
  currentCyto[0]=ecArray[index].PAF;
...
void EC::activate(float PAFmult, float IL8mult, vector<EC>&ecArray){
	ec_roll++;
	ec_stick++;
	PAF=cytokineProductionRule(0,id,0,0,PAFmult, ecArray);
	IL8=cytokineProductionRule(1,id,0,0,IL8mult, ecArray);
//	cout<<"id="<<id<<"PAF="<<PAF<<"\n";
}
...
void EC::ECfunction(float oxyHeal,float PAFmult, float IL8mult, vector<EC>&ecArray){
	if((endotoxin>=1)||(oxy<60)){
		ec_activation=1;}
	if(ec_activation==1){
		activate(PAFmult, IL8mult, ecArray);}
...
void EC::patch_inj_spread(float oxyHeal, float PAFmult, vector<EC>&ecArray){
	...
	if((oxy<60)&&(oxy>30)){ //ischemia
		...
		PAF=cytokineProductionRule(2,id,0,0,PAFmult, ecArray);
		for(i=0;i<8;i++){
...
void pmn::pmn_function(int pmnID, float IL1ramult, float TNFmult, float IL1mult, int(&cellGrid)[101][101], vector<EC>& ecArray, vector<pmn>& pmnArray){
	int roll,x,y,id;
	float TNF,PAF,IL1;
	x=xLoc;
	y=yLoc;
	...
	if(wbc_migrate>0){
		pmn_burst(pmnID, TNFmult, IL1ramult, cellGrid, ecArray, pmnArray);}
	else{
		...
		if(TNF+PAF>1){
			wbc_stick=IL1;
			ecArray[id].IL1ra=cytokineProductionRule(3,id,0,0,IL1ramult, ecArray);
		}
		if((wbc_stick>=1)&&(ecArray[id].ec_stick>=100)){
			wbc_migrate=max(float(0),(ecArray[id].TNF+ecArray[id].IL1+ecArray[id].GCSF-HypoInflamParam1*ecArray[id].IL10));}
		pmn_age--;
		if(pmn_age<0){
			x=xLoc;
			y=yLoc;
...
void pmn::pmn_burst(int pmnID, float TNFmult, float IL1mult, int (&cellGrid)[101][101], vector<EC>& ecArray, vector<pmn>&pmnArray){
	int x,y,id;

	x=xLoc;
	y=yLoc;
	id=y*xDim+x;
	ecArray[id].cytotox=max(float(0),HypoInflamParam2*ecArray[id].TNF);
	// ecArray[id].oxy=100;
	// ecArray[id].ec_roll=0;
	// ecArray[id].ec_stick=0;
	// ecArray[id].ec_migrate=0;
	ecArray[id].TNF=cytokineProductionRule(5,id,0,0,TNFmult, ecArray);
	ecArray[id].IL1=cytokineProductionRule(6,id,0,0,IL1mult, ecArray);
	pmn_age=pmn_pcd;
	pmn_pcd=pmn_pcd-1+max(float(0),cytokineComboRule(7,id,0,0, ecArray)/100);
	if(pmn_age<0){
		x=xLoc;
		y=yLoc;
		pmnArray.erase(pmnArray.begin()+pmnID);
...
void mono::mono_function(int index, float IL1ramult, float sTNFrmult, float sIL1rmult, float GCSFmult, float IL8mult, float IL12mult, float IL10mult, float IL1mult, float TNFmult, int(&cellGrid)[101][101], vector<EC>& ecArray, vector<mono>& monoArray){
	int x,y,id;
	...
	ecArray[id].IL1ra=cytokineProductionRule(8,id,TNFr,IL_1r,IL1ramult, ecArray);
	ecArray[id].sTNFr=cytokineProductionRule(9,id,TNFr,IL_1r,sTNFrmult, ecArray);
	ecArray[id].sIL1r=cytokineProductionRule(10,id,TNFr,IL_1r,sIL1rmult, ecArray);

	activation=ecArray[id].endotoxin+ecArray[id].PAF+ecArray[id].IFNg-HypoInflamParameter3*ecArray[id].IL10;
	
	if(activation>0){
		ecArray[id].GCSF=cytokineProductionRule(12,id,TNFr,IL_1r,GCSFmult, ecArray);
		ecArray[id].IL8=cytokineProductionRule(13,id,TNFr,IL_1r,IL8mult, ecArray);
		ecArray[id].IL12=cytokineProductionRule(14,id,TNFr,IL_1r,IL12mult, ecArray);
		ecArray[id].IL10=cytokineProductionRule(15,id,TNFr,IL_1r,IL10mult, ecArray);
		ecArray[id].IL1=cytokineProductionRule(16,id,TNFr,IL_1r,IL1mult, ecArray);
		ecArray[id].TNF=cytokineProductionRule(17,id,TNFr,IL_1r,TNFmult, ecArray);
	...
	if(activation<0){
		ecArray[id].IL10=cytokineProductionRule(18,id,TNFr,IL_1r,IL10mult, ecArray);
	}
	if(wbc_migrate==1){
		heal(id, ecArray);}
	if(wbc_roll==1){
...
void TH0::TH0function(int index, int (&cellGrid)[101][101], vector<EC>&ecArray, vector<TH0>&TH0array, vector<TH1>&TH1array, vector<TH2>&TH2array){
	int id;
//	cout<<"activation="<<activation<<"\n";
	id=yLoc*xDim+xLoc;
	if(ecArray[id].IL12+ecArray[id].IL4>0){
		proTH1=cytokineProductionRule(20,id,0,0,1, ecArray)*100;
		proTH2=cytokineProductionRule(21,id,0,0,1, ecArray)*100;
...
void TH1::TH1function(int index, float IFNgmult, int (&cellGrid)[101][101], vector<EC>& ecArray, vector<TH1>& TH1array){
	...
	if(ecArray[id].IL12>0){
		ecArray[id].IFNg=cytokineProductionRule(22,id,0,0,IFNgmult, ecArray);
	}
	wiggle(&orientation);
	move(orientation, &xLoc, &yLoc, cellGrid);
	TH1_age--;
...
void TH2::TH2function(int index, float IL4mult, float IL10mult, int (&cellGrid)[101][101], vector<EC>& ecArray, vector<TH2>& TH2array){
	int id;

	id=yLoc*xDim+xLoc;

	if(ecArray[id].IL10>0){
		ecArray[id].IL4=cytokineProductionRule(23,id,0,0,IL4mult, ecArray);
		ecArray[id].IL10=cytokineProductionRule(24,id,0,0,IL10mult, ecArray);
	}
	wiggle(&orientation);
	move(orientation, &xLoc, &yLoc, cellGrid);
	TH2_age--;
